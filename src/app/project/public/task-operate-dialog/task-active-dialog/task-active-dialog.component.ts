...
@Component({
  selector: 'pms-task-active-dialog',
  templateUrl: './task-active-dialog.component.html',
  styleUrls: ['./task-active-dialog.component.scss']
})
export class TaskActiveDialogComponent implements OnInit {
  ...
  onOk(event) {
    ...
    if (this._form.valid) {
      ...
      this.taskApi.active(this.task.id, value).subscribe(
        ok => {
