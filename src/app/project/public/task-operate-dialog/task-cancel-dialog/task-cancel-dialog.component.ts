...
@Component({
  selector: 'pms-task-cancel-dialog',
  templateUrl: './task-cancel-dialog.component.html',
  styleUrls: ['./task-cancel-dialog.component.scss']
})
export class TaskCancelDialogComponent implements OnInit {
  ...
  onOk(event) {
    ...
    if (this._form.valid) {
      ...
      this.taskApi.cancel(this.task.id, value).subscribe(
        ok => {
