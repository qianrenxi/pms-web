...
@Component({
  selector: 'pms-task-restart-dialog',
  templateUrl: './task-restart-dialog.component.html',
  styleUrls: ['./task-restart-dialog.component.scss']
})
export class TaskRestartDialogComponent implements OnInit {
  ...
  onOk(event) {
    ...
    if (this._form.valid) {
      ...
      this.taskApi.restart(this.task.id, value).subscribe(
        ok => {
