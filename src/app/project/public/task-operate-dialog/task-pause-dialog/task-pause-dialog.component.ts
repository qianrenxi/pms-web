...
@Component({
  selector: 'pms-task-pause-dialog',
  templateUrl: './task-pause-dialog.component.html',
  styleUrls: ['./task-pause-dialog.component.scss']
})
export class TaskPauseDialogComponent implements OnInit {
  ...
  onOk(event) {
    ...
    if (this._form.valid) {
      ...
      this.taskApi.pause(this.task.id, value).subscribe(
        ok => {
